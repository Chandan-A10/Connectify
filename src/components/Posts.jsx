...
import { SendOutlined, UserOutlined, LikeOutlined, CommentOutlined } from '@ant-design/icons'
...
const Posts = ({ user }) => {
    ...
    const handleAddComment = (postid) => {
        if (cmtRef.current.input.value.trim()) {
            updateComments(postid, user.name, cmtRef.current.input.value)
                ...
                <div>
                    <div style={{ display: 'flex', marginTop: '-1%', alignItems: 'center', height: 50 }}>
                        ...
                        <Form style={{ display: 'flex', width: '100%' }}>
                            <Input ref={postRef} className={post.input} style={{ backgroundColor: '#27282F', color: '#f2f3f5', border: 'none', marginLeft: '1%', borderRadius: '20px' }} placeholder="Add a new posts"></Input><Button size='large' style={{ background: 'transparent', color: 'white', border: 'none' }} icon={<SendOutlined />}></Button>
                        </Form>
                    </div>
